...
def update_user(current_user, public_id):
    ...
    update_username = User.query.filter_by(
        public_id=public_id).update(dict(username=data['username']))
    db.session.commit()

    hashed_password = generate_password_hash(data['password'], method='sha256')
    update_password = User.query.filter_by(
        public_id=public_id).update(dict(password=hashed_password))
    db.session.commit()

    update_email = User.query.filter_by(
        public_id=public_id).update(dict(email=data['email']))
    db.session.commit()
